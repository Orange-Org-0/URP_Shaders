...
public sealed class FoliageQuadMaterialAssigner : MonoBehaviour
{
    [SerializeField]
    [Tooltip("要分配给所有 Foliage Quad 的共享材质。")]
    private Material targetMaterial = null;

    public Material TargetMaterial => targetMaterial;

    /// <summary>
    /// 查找当前物体及其所有后代中，名称同时包含 foliage 和 quad 的 MeshRenderer。
    /// 未激活物体也会被查找。
    /// </summary>
    public List<MeshRenderer> FindMatchingRenderers()
    {
        ...
        foreach (MeshRenderer renderer in renderers)
        {
            ...
            if (objectName.IndexOf("foliage", StringComparison.OrdinalIgnoreCase) >= 0 &&
