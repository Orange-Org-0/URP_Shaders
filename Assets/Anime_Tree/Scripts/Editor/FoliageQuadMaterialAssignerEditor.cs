...
[CustomEditor(typeof(FoliageQuadMaterialAssigner))]
[CanEditMultipleObjects]
public sealed class FoliageQuadMaterialAssignerEditor : Editor
{
    ...
    private void AssignMaterials()
    {
        ...
        Undo.SetCurrentGroupName("分配 Foliage Quad 材质");
        ...
        {
            var assigner = (FoliageQuadMaterialAssigner)selectedTarget;
            ...
            foreach (MeshRenderer renderer in matches)
            {
                ...
                Undo.RecordObject(renderer, "分配 Foliage Quad 材质");
            ...
            Debug.Log(
                $"[{assigner.name}] Foliage Quad 材质处理完成：找到 {matches.Count} 个，" +
                $"修改 {changed} 个，跳过 {skipped} 个。",
                assigner);
        }
        ...
        {
            Debug.Log(
                $"Foliage Quad 批量处理汇总：找到 {totalFound} 个，修改 {totalChanged} 个，" +
