...
[CustomEditor(typeof(GrassQuadMaterialAssigner))]
[CanEditMultipleObjects]
public sealed class GrassQuadMaterialAssignerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        {
            EditorGUILayout.HelpBox("请先指定要分配给 Grass Quad 的材质。", MessageType.Warning);
        }
    ...
    private void AssignMaterials()
    {
        ...
        Undo.SetCurrentGroupName("分配 Grass Quad 材质");
        ...
        {
            var assigner = (GrassQuadMaterialAssigner)selectedTarget;
            ...
            foreach (MeshRenderer renderer in matches)
            {
                ...
                Undo.RecordObject(renderer, "分配 Grass Quad 材质");
            ...
            Debug.Log(
                $"[{assigner.name}] Grass Quad 材质处理完成：找到 {matches.Count} 个，修改 {changed} 个，跳过 {skipped} 个。",
                assigner);
        }
        ...
        {
            Debug.Log(
                $"Grass Quad 批量处理汇总：找到 {totalFound} 个，修改 {totalChanged} 个，" +
